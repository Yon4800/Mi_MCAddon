...
world.afterEvents.playerBreakBlock.subscribe((event) => {
      ...
      if (!isVeinMiningInProgress.has(pKey)) {
        ...
        system.run(() => {
          try {
            ...
            if (curHeld) {
              const enchantComp = curHeld.getComponent("minecraft:enchantable") as any;
              if (enchantComp) {
  ...
  // 4. カスタムツール耐久値消費 (ota, otaku_cry, igyo_tool の単体破壊用)
  if (player.gameMode !== "creative") {
    try {
      if (!equippable) return;
      const curHandItem = equippable.getEquipment("Mainhand" as any);
      if (!curHandItem) return;

      const typeId = curHandItem.typeId;
      if (typeId === "mi:ota" || typeId === "mi:otaku_cry" || typeId === "mi:igyo_tool") {
        const durability = curHandItem.getComponent("minecraft:durability") as any;
        if (durability) {
          let unbreakingLevel = 0;
          const enchantable = curHandItem.getComponent("minecraft:enchantable") as any;
          if (enchantable) {
          ...
          if (Math.random() < damageChance) {
            if (durability.damage + 1 >= durability.maxDurability) {
              ...
              durability.damage += 1;
              equippable.setEquipment("Mainhand" as any, curHandItem);
            }
          }
        }
      }
...
const isVeinMiningInProgress = new Set<string>(); // avoid recursion
